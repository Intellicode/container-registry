...
Initialize the container registry project with Deno 2.x and Hono framework. Set
up the directory structure, configuration files, and basic application entry
point as defined in the PRD architecture.
...
- [ ] `src/config.ts` loads configuration from environment variables with
      defaults
...
- Configuration should support `REGISTRY_HOST` and `REGISTRY_PORT` environment
  variables
