...
Implement chunked/streaming blob uploads where clients can send data in multiple PATCH requests before completing with PUT. This is essential for large layers and resumable uploads.
...
- [ ] `PUT /v2/<name>/blobs/uploads/<uuid>?digest=<digest>` completes chunked upload:
...
- `Content-Range` parsing: `bytes <start>-<end>/<total>` or `bytes <start>-<end>`
